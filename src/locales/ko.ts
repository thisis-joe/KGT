export default {
  nav: {
    home: '홈',
    company: '회사소개',
    business: '사업영역',
    store: '스토어',
    support: '고객지원',
    contact: '문의하기',
    storeNotice:
      '온라인 스토어는 현재 준비 중입니다.\n문의 사항이 있으시면 우측 상단의 \"문의하기\"를 이용해 주시기 바랍니다.',
  },
  hero: {
    title: '점·접착 소재의\n글로벌 리더',
    subtitle:
      'KGT는 점·접착 소재 및 기능성 테이프 전문기업으로,\n축적된 기술과 경험을 바탕으로\n소재·부품 업계의 글로벌 리더로 성장하고 있습니다.',
    cta: '자세히 보기',
  },
  overview: {
    title: '회사 개요',
    description:
      '저희 KGT는 점·접착 소재 및 기능성 테이프 전문기업으로, 축적된 다양한 기술과 경험을 바탕으로 소재·부품 업계의 글로벌 리더로 성장하고자 합니다. 변화와 혁신을 추구하며, 뚜렷한 목표와 고객 중심의 가치를 바탕으로 끊임없이 도전하고 발전해 나가고 있습니다.',
    ceo: '대표이사',
    ceoName: '주창석',
    founded: '설립일',
    foundedDate: '2021.7.2',
    business: '사업분야',
    businessAreas: '필름 & 테이프 사업 / 원재료 사업 / 기술서비스사업',
  },
  vision: {
    title: 'Vision',
    statement: '소재&부품업계 Global Leader',
  },
  coreValues: {
    title: 'Core Values',
    sectionTitle: 'KGT를 선택해야 하는 이유',
    creativity: {
      title: '창의 Creativity',
      subtitle: '긍정성 / Open Mind / 실력',
      description:
        '기존의 방식에서 벗어나 긍정적으로 수용하고 열린 사고로 고객을 위한 새로운 가치를 창출해 나가겠습니다.',
    },
    innovation: {
      title: '혁신 Innovation',
      subtitle: '목표지향 / 고객지향 / 추진력',
      description:
        '높은 목표 지향적 사고와 빠른 추진력으로 고객을 위한 창의적인 발상을 측정 가능한 성과로 만들어 내기 위해 상호협력하며 실패를 두려워하지 않고 추진해 나가겠습니다.',
    },
    challenge: {
      title: '도전 Challenge',
      subtitle: '목표 / 미래 / 환경',
      description:
        '고객의 니즈와 기술의 변화를 한발 앞서 파악하여 풍요로운 미래를 만들어가기 위해 스스로의 한계에 도전해 나가겠습니다.',
    },
  },
  business: {
    title: '핵심 사업 포트폴리오',
    subtitle: '고객 요구에 맞는 점·접착 솔루션을 제공합니다.',
    downloadCatalog: '카탈로그 다운로드',
    explore: '자세히 보기',
    filmTape: {
      title: '필름 & 테이프 사업',
      products: [
        {
          name: 'Hot Melt 필름',
          application: '반도체 폴리싱 패드',
        },
        {
          name: '보호필름',
          application: 'Mobile Phone',
        },
        {
          name: '양면테이프',
          application: 'Mobile Phone',
        },
        {
          name: 'EMI & EMC 도전성 테이프',
          application: 'Mobile Phone',
        },
      ],
    },
    materials: {
      title: '원료 사업',
      products: [
        {
          name: '광고용 실사소재 접착제',
          application: '광고용 소재',
        },
        {
          name: '산업용 점접착제',
          application: '선박용 단열제 난연 테이프용',
        },
        {
          name: '기능성 점착제',
          application: '비산방지용 / Blue Light Cut 점착용',
        },
        {
          name: '기능성 도료',
          application: 'AF, AG Hard Coating 도료용 / 프라이머용',
        },
      ],
    },
    service: {
      title: '기술 서비스',
      items: ['점접착제 공동개발 및 자문', '점접착필름 제조조건 자문'],
    },
  },
  store: {
    title: '공식 스토어',
    description: 'KGT의 제품을 네이버 스마트스토어에서 만나보세요',
    button: '스토어 바로가기',
  },
  contact: {
    title: '문의하기',
    subtitle: '궁금하신 사항이 있으시면 언제든지 문의해주세요.',
    form: {
      name: '이름',
      company: '회사명',
      email: '받으실 이메일',
      phone: '연락처',
      subject: '제목',
      message: '문의내용',
      submit: '문의하기',
      namePlaceholder: '성함을 입력해주세요',
      companyPlaceholder: '회사명을 입력해주세요',
      emailPlaceholder: 'client.kgt.web@gmail.com',
      phonePlaceholder: '연락처를 입력해주세요',
      subjectPlaceholder: '제목을 입력해주세요',
      messagePlaceholder: '문의하실 내용을 입력해주세요',
      success: '문의가 성공적으로 전송되었습니다.',
      error: '문의 전송에 실패했습니다. 다시 시도해주세요.',
    },
    info: {
      title: '연락처 정보',
      headquarters: 'Head Office / R&D Center',
      hqAddress:
        '2F, 40, Hasinjungang-ro 54beon-gil (Jangnim-dong), Saha-gu, Busan, Republic of Korea',
      branch: 'Gyeonggi Sales Office',
      branchAddress:
        'B-3321, Geumgang Penterium IX Tower, 27 Dongtancheomdansaneop 1-ro, Hwaseong-si, Gyeonggi-do, Republic of Korea',
      phone: 'Tel',
      fax: 'Fax',
      email: '이메일',
      website: '홈페이지',
    },
  },
  contactPage: {
    nav: {
      companyProfile: '회사 소개',
      product: '제품',
      qualityControl: '품질 관리',
      contactUs: '문의하기',
      quickLinks: '바로가기',
      products: '제품',
      sustainability: '지속가능경영',
      legal: '법적 고지',
      privacyPolicy: '개인정보처리방침',
      termsOfService: '이용약관',
      sitemap: '사이트맵',
    },
    hero: {
      title: '문의하기',
      subtitle:
        '기능성 테이프 및 점·접착 소재 전문기업 KGT가 신속하고 신뢰할 수 있는 답변을 제공합니다.',
    },
    locations: {
      title: '사업장 안내',
      headOffice: '본사 / R&D 센터',
      headOfficeAddress: '부산광역시 사하구 하신중앙로54번길 40(장림동), 2층',
      branchOffice: '경기 영업사무소',
      branchOfficeAddress: '경기도 화성시 동탄첨단산업1로 27, 금강펜테리움 IX타워 B-3321',
      mapButton: '지도 보기',
      mapAlt: '사업장 위치 지도',
    },
    form: {
      title: '문의 접수',
      description: '아래 양식을 작성해 주세요. * 표시는 필수 입력 항목입니다.',
      yourName: '이름',
      companyName: '회사명',
      emailHint: '비워두면 기본 발신 주소로 전송됩니다.',
      subject: '제목',
      message: '문의 내용',
      privacyPrefix: '아래 ',
      privacyPolicy: '개인정보처리방침',
      privacyDescription: '에 동의하며, 문의 처리 목적에 한해 개인정보를 제공합니다.',
      submit: '문의 보내기',
      sending: '전송 중...',
      success: '문의가 정상적으로 접수되었습니다.',
      privacyError: '문의 전 개인정보처리방침 동의가 필요합니다.',
      fallback: '서버 연결에 실패했습니다. 아래 이메일로 직접 문의해 주시기 바랍니다.',
      mailConfigError:
        '메일 서버 설정이 완료되지 않았습니다. 관리자에게 문의하시거나 아래 이메일을 이용해 주시기 바랍니다.',
      subjects: {
        productInquiry: '제품 문의',
        quotationRequest: '견적 요청',
        technicalSupport: '기술 지원',
        partnershipProposal: '협업 제안',
        other: '기타',
      },
    },
    footer: {
      companyDescription:
        'KGT는 산업용 기능성 테이프와 첨단 점·접착 솔루션을 제공하는 신뢰 기반의 글로벌 파트너입니다.',
      copyright: '© 2023 KGT Corporation. All rights reserved.',
      linkedin: '링크드인',
      twitter: 'X',
    },
  },
  footer: {
    company: 'KGT',
    description:
      'KGT는 산업용 기능성 테이프와 첨단 점·접착 솔루션을 제공하는 신뢰 기반의 글로벌 파트너입니다.',
    quickAccess: '바로가기',
    naverStore: '네이버 스마트스토어',
    sendSuggestion: '기능 제안하기',
    contactTitle: '사업장 안내',
    headOffice: '본사 / R&D 센터',
    headOfficeAddress: '부산광역시 사하구 하신중앙로54번길 40(장림동), 2층',
    branchOffice: '경기 영업사무소',
    branchOfficeAddress: '경기도 화성시 동탄첨단산업1로 27, 금강펜테리움 IX타워 B-3321',
    copyright: '© 2023 KGT Global. All rights reserved.',
    privacyPolicy: '개인정보처리방침',
    termsOfService: '이용약관',
    close: '닫기',
    privacyContent1: '문의 응답 및 요청 서비스 제공에 필요한 최소한의 정보만 수집합니다.',
    privacyContent2:
      '수집된 데이터는 업무 커뮤니케이션 목적으로만 사용되며, 법적 근거 없이 제3자에게 공유되지 않습니다.',
    privacyContent3: '데이터 요청 또는 정정은 공식 문의 채널을 통해 회사에 연락해 주시기 바랍니다.',
    termsContent1:
      '본 웹사이트 콘텐츠는 비즈니스 정보 제공 목적으로 제공되며, 사전 통지 없이 업데이트될 수 있습니다.',
    termsContent2:
      '사전 서면 동의 없이 콘텐츠를 무단 복사, 재배포 또는 상업적으로 사용하는 것은 제한됩니다.',
    termsContent3: '서비스 가용성은 유지보수 또는 운영 요구 사항에 따라 변경될 수 있습니다.',
    featureSuggestion: '기능 제안',
    suggestionName: '이름',
    suggestionNamePlaceholder: '이름을 입력하세요',
    suggestionEmail: '받으실 이메일',
    suggestionEmailPlaceholder: 'client.kgt.web@gmail.com',
    suggestionLabel: '제안 내용',
    sendToDeveloper: '개발자에게 보내기',
  },
  stats: {
    title: '숫자로 보는 KGT',
    founded: '설립년도',
    clients: '거래처',
    products: '취급 제품',
    satisfaction: '고객 만족도',
  },
  products: {
    title: '제품 포트폴리오',
    subtitle: 'KGT의 다양한 점·접착 소재 제품을 만나보세요',
    categories: {
      all: '전체',
      film: '필름',
      tape: '테이프',
      adhesive: '접착제',
    },
    items: {
      hotmelt: {
        title: 'Hot Melt 필름',
        description: '반도체 폴리싱 패드용 고기능성 열접착 필름',
      },
      protective: {
        title: '보호필름',
        description: '모바일 디바이스용 고품질 표면 보호 필름',
      },
      doubleSided: {
        title: '양면테이프',
        description: '모바일 기기 조립용 강력 양면 접착 테이프',
      },
      emi: {
        title: 'EMI & EMC 도전성 테이프',
        description: '전자파 차폐용 도전성 기능성 테이프',
      },
      advertising: {
        title: '광고용 실사소재 접착제',
        description: '옥외 광고용 고내구성 접착 솔루션',
      },
      industrial: {
        title: '산업용 점접착제',
        description: '선박용 단열제 및 난연 테이프용 특수 접착제',
      },
    },
  },
};
